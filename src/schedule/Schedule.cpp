...
{
public:
    ScheduleTimer(std::chrono::milliseconds timeout, Schedule::EntryData& data)
        : Timer(timeout), mData(data)
    {
    }
...
void Schedule::realize()
{
    ...
    for (auto& entry : mEntries) {
        ...
        auto makeTimer = [&data, &loop](int min) {
            if (min > 0) {
                ...
                auto timer = std::make_shared<ScheduleTimer>(milliseconds{min * 60 * 1000} - delta, data);
